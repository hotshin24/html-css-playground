 ...
 *
 * 미리보기(3열)와 별개로 운영한다. 격리 수준은 같고(`allow-same-origin` 단독
 * + CSP), 이 문서만 고정 폭과 화면 밖 배치를 쓴다. 열 너비를 드래그해도
 * 측정값이 흔들리지 않아야 하기 때문이다.
 *
 * 예전에는 미리보기가 `sandbox=""`여서 "완화된 오리진을 갖는 문서의 수명이
 * 판정 순간으로 제한된다"를 근거로 삼았다. 지금은 두 문서가 같은 조합을
 * 쓰므로 그 근거는 성립하지 않는다. 안전은 수명이 아니라 권한에서 온다.
 * `allow-scripts`가 없으면 스크립트가 실행될 경로가 없다.
 */
